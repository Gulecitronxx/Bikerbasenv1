...
**Remaining copy conflict, same class:** the generated intro on the licence landing pages (e.g. `koerekort-a2.html`: *"…der må køres på et A2-kørekort"*) still makes an affirmative legality claim from power alone. The text is produced by the build pipeline, so the fix lives in the generator, not the page. This is now the top open integrity item.
...
### Next
- **Fix the licence landing-page intros** (3.1) — the remaining "må køres på A2" overclaim, generated by the build pipeline
...
1. **Fix the licence landing-page intros** (3.1) — the last live "misleading precision" copy; the fix lives in the build pipeline.
2. **Formalise the aggregation layer** (4.4–4.5): written consent register per source; counsel confirmation of the GDPR position (minimised mode is already enforced in code); keep private-seller sources minimised until confirmed.
3. **Complete the dealer funnel:** claim → account → subscription conversion, then feeds and profile pages. Build the automated claim-verification paths (domain match, code on site) before claim volume makes manual approval a bottleneck or a risk.
4. **A2 model database and slutseddel generator** — owned content and utility that work regardless of inventory mix, and the backbone for *A2 bekræftet*.
5. **Grow supply:** more consented sources, dealer feeds via CVR outreach, first native private listings.
6. **Drive traffic** to real pages; expand email capture beyond the live søgeagenter.
7. **Donor bikes and used parts** as a second supply and traffic engine.
8. **Monetisation infrastructure** on top of the claim funnel.
